...
public class IsoDate {
	...
	if ((type & DATE) != 0) {
	    ...
	    c.set (Calendar.MONTH, Integer.parseInt 
		   (text.substring (4,6)) - 1 + Calendar.JANUARY); //5,7
	    c.set (Calendar.DAY_OF_MONTH, Integer.parseInt 
		   (text.substring (6,8))); //original - 8,10, change for the resonse pattern yyyymmddThh:mm:ss
	    
	    if (type == DATE_TIME) 
		text = text.substring (9); //11 original 
	}
